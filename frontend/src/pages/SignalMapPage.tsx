...
export function SignalMapPage() {
  ...
  const { signals, totalSignals, heatmap, categoryCounts, sourceCounts, updatedAt, isLoading, error, isSparse, isQuiet } =
    useSignalMap();
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-4xl text-[14px] leading-6 text-[var(--e-text-secondary)]">
              Raw OSINT relationship surface for active signals, categories, source families, and convergence cells. This is the event-source companion to the immersive World Monitor globe.
            </p>
          </div>
        ...
        <div
          className={clsx(
            ...
            criticalSignals >= 4
              ...
              : 'border border-[color:oklch(0.525_0.155_260_/_0.18)] bg-[color:oklch(0.525_0.155_260_/_0.08)]',
          )}
        >
          {criticalSignals >= 4 ? (
            <AlertTriangle className="h-4 w-4 shrink-0 text-[var(--e-red-600)]" />
          ) : (
            <Network className="h-4 w-4 shrink-0 text-[var(--e-purple-700)]" />
          )}
          <span className={clsx('font-semibold', criticalSignals >= 4 ? 'text-[var(--e-red-600)]' : 'text-[var(--e-purple-700)]')}>
            {criticalSignals >= 4 ? 'Escalated signal network' : 'Live signal topology'}
          </span>
          <span className="text-[var(--e-text-secondary)]">
            {totalSignals} total signals · {heatmap.length} convergence cell{heatmap.length === 1 ? '' : 's'} · updated {updatedAt ? formatTimeAgo(updatedAt) : 'recently'}
          </span>
        </div>

        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <KpiCard label="Live Signals" value={totalSignals} detail={`${filteredSignals.length} currently visible`} tone={totalSignals > 40 ? 'warning' : undefined} />
          <KpiCard label="Critical / High" value={criticalSignals} detail="Escalated network pressure" tone={criticalSignals > 0 ? 'danger' : 'success'} />
          <KpiCard label="Source Families" value={Object.keys(sourceCounts).length} detail="Distinct live source layers" />
          <KpiCard label="Convergence Cells" value={heatmap.length} detail="Geo co-location alerts" tone={heatmap.length > 5 ? 'warning' : undefined} />
        </div>
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Search signals, regions, sources"
