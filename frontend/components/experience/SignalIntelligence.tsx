...
// Transform API signal response (snake_case) to component format (camelCase)
const transformSignal = (apiSignal: any): SignalItem => {
  ...
  return {
    ...
    label: apiSignal.signal_type,
    message: apiSignal.message,
    level: apiSignal.level,
    signalType: apiSignal.signal_type,
    agentName: apiSignal.agent_id,
    agentColor: agentColors[apiSignal.agent_id] || "#6366f1",
  };
};

// Transform timeline health response
const transformTimelineHealth = (apiHealth: any): TimelineHealth => ({
  timelineName: apiHealth.timeline_name,
  status: apiHealth.status,
  timeToCollapse: apiHealth.time_to_collapse,
  realityAlignment: Math.round(apiHealth.reality_alignment * 100),
  stabilityIndex: Math.round(apiHealth.stability_index * 100),
  decayRate: apiHealth.decay_rate * 100,
  sabotagePressure: Math.round(apiHealth.sabotage_pressure * 100),
  shieldCoverage: Math.round(apiHealth.shield_coverage * 100),
  netMomentum: apiHealth.net_momentum,
});

// Transform market forces response
const transformMarketForces = (apiForces: any): MarketForce => ({
  marketId: apiForces.market_id,
  marketName: apiForces.market_name,
  sabotageAmount: apiForces.sabotage_amount,
  shieldAmount: apiForces.shield_amount,
  currentPrice: apiForces.current_price,
  topSaboteurs: apiForces.top_saboteurs?.map((s: any) => ({
    agentName: s.agentName || s.agent_name,
    amount: s.amount,
    isUser: s.isUser || s.is_user,
  })) || [],
  topDefenders: apiForces.top_defenders?.map((d: any) => ({
    agentName: d.agentName || d.agent_name,
    amount: d.amount,
    isUser: d.isUser || d.is_user,
  })) || [],
});

// Enhanced fetcher with transformation
const fetchSignals = async (url: string): Promise<SignalItem[]> => {
  const res = await fetch(url);
  const data = await res.json();
  return Array.isArray(data) ? data.map(transformSignal) : [];
};

const fetchTimelineHealth = async (url: string): Promise<TimelineHealth> => {
  const res = await fetch(url);
  const data = await res.json();
  return transformTimelineHealth(data);
};

const fetchMarketForces = async (url: string): Promise<MarketForce> => {
  const res = await fetch(url);
  const data = await res.json();
  return transformMarketForces(data);
};
...
export default function SignalIntelligence({
  ...
  // Fetch signals from new Operations API
  ...
  // Fetch timeline health from new Operations API
  const { data: timelineHealth, mutate: mutateHealth } = useSWR<TimelineHealth>(
    `${API_BASE}/api/timeline-health/default`,
    fetchTimelineHealth,
    {
  ...
  // Fetch market forces from new Operations API
  const { data: marketForces, mutate: mutateForces } = useSWR<MarketForce>(
    `${API_BASE}/api/market-forces/${selectedMarket}`,
    fetchMarketForces,
    {
      refreshInterval: 10000,
