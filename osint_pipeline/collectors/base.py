...
class BaseCollector(ABC):
    ...
    FREE_SOURCE_CONFIDENCE_CAP: float = 0.7
    ...
    @classmethod
    def should_cap_confidence(cls, registry_source: RegistrySource) -> bool:
        """Return True if the source warrants a confidence ceiling.

        Free sources with scrape-based access or latest-only revision
        policies are inherently less stable.
        """
        return (
            registry_source.access_surface == "portal_scrape"
            or registry_source.revision_policy == "latest_only"
        )
    ...
    def collect(
        ...
        try:
            ...
            # Free source confidence cap
            if registry_source is not None and self.should_cap_confidence(registry_source):
                cap = self.config.get("confidence_cap", self.FREE_SOURCE_CONFIDENCE_CAP)
    ...
    def to_gap_report(
        ...
        return GapReport(
            ...
            if result.status != CollectionStatus.NOT_FOUND
            else FreshnessState.NO_DATA,
        )
