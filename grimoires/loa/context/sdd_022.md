...
Wizard Step 1: Template Selection
  ...
  -> Step 2 inquiry question is written within that template-defined frame
...
### 5.3 Wizard Template-First Refactor

In `CreateInvestigationWizard.tsx`:

- Remove the static `TEMPLATES` array
- Replace with `useInvestigationTemplates()` hook call
- Make template selection the first wizard step
- Render template selection as a dropdown backed by the fetched API list
...
- Non-template fields (`theatre_id`, signal-specific domain filters) continue to be populated directly from URL params — they are not affected by this change.
- `construct_id` is not exposed as a free-text field in the wizard. If a trusted launch context provides a construct binding, it is surfaced read-only as inherited context. Manual construct selection remains deferred until a backend construct registry/list endpoint exists.
...
export async function createInvestigation(body: {
  theatre_id?: string;
  construct_id?: string; // inherited trusted context only; not a free-text wizard field
  inquiry_class?: string;
