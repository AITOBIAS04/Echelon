...
**Version**: 2.0
**Date**: 2026-03-02
**Sprint 1**: COMPLETED (63 tests, 12 files)
...
| Trades execute with correct cost | `trade_cost()` applied, x vector updated atomically |
| Position tracking is correct | Shares, net cashflow, realised P&L accumulate correctly |
| Settlement pays winning shares 1:1 | Settlement payout = `position.shares[winning_outcome]` |
| Market maker loss bounded | `market_maker_pnl >= -worst_case_loss(b, n)` |
| End-to-end lifecycle works | Create → commit → trade → resolve → settle with correct P&L |
| 25+ new Sprint 2 tests pass | Trading, positions, settlement invariants, end-to-end |
...
- **Agent Architecture** — executes trades via `TradingEngine` (Sprint 2 — this sprint)
- **On-chain Settlement** — commits market outcomes to Base Sepolia (010b)
- **MCP Surface** — `echelon_status` market-state integration (010b)

**Sprint 1**: COMPLETED — LMSR core, lifecycle, commitment hash (63 tests).
**Sprint 2**: Trade execution, agent positions, resolution, settlement.
...
### 4.6 Trade Execution (Sprint 2)

**File**: `backend/market/trading.py`

The transactional layer on top of the LMSR core. Agents submit trades, the engine validates and executes them. Not pure — mutates `MarketState.x`.
...
class Trade:
    ...
    cost: float                 # computed by engine
    pre_trade_prices: list[float]
    post_trade_prices: list[float]
    timestamp: str

class TradingEngine:
    def execute_trade(self, market, agent_id, outcome_index, shares) -> Trade
    def quote(self, market, outcome_index, shares) -> float
```

**Trade semantics (010a)**: One-hot deltas — each trade targets a single outcome. `delta[outcome_index] = shares`, zero elsewhere. No cross-outcome bundles.

**Validation rules**:
- Market must be in TRADING phase (`TradingHalted` if not)
- `outcome_index` must be valid (0 to n_outcomes-1)
- `shares` must be non-zero
- Sell inventory constraint: if `shares < 0`, agent must hold `abs(shares)` in position (`InsufficientShares`)
- Agent must have sufficient balance to cover cost (`InsufficientBalance`)
- No naked shorts

**Execution is atomic**: If any validation fails, no state mutates.

### 4.7 Agent Position Tracking (Sprint 2)
...
class AgentPosition:
    ...
    net_cashflow: float         # sum of signed trade costs
    realised_pnl: float         # P&L from closed positions
    trade_count: int

class PositionManager:
    def get_position(self, agent_id, market_id) -> AgentPosition
    def update_position(self, agent_id, market_id, trade) -> AgentPosition
    def compute_unrealised_pnl(self, position, current_prices) -> float
    def compute_settlement_payout(self, position, resolved_outcome) -> float
```

**Position invariants**:
- `net_cashflow` = sum of all signed `trade.cost` for this agent in this market
- Settlement payout for outcome k = `position.shares[k] * 1.0` (winning shares pay 1:1)
- Losing shares pay 0.0. No partial outcomes. No fees in 010a.
- `realised_pnl` at settlement = `settlement_payout - net_cashflow`

### 4.8 Resolution and Settlement (Sprint 2)

**File**: `backend/market/resolution.py`

```python
class ResolutionEngine:
    def resolve(self, market, winning_outcome) -> MarketState
    def settle(self, market, position_manager) -> SettlementReport
...
- `total_payout = sum(agent.payout for all agents)`
- `market_maker_pnl = sum(agent.net_cashflow) - total_payout`
- `market_maker_pnl >= -worst_case_loss(b, n)` (bounded loss guarantee)
- `agent.realised_pnl == agent.payout - agent.net_cashflow`
- Settlement report hash is deterministic

> Source: echelon_cycle_010a.md (Sprint 2)
...
- Trade execution engine (`TradingEngine`) — validate, execute, atomic state mutation
- Pre-trade quote function
- Agent position tracking (`PositionManager`) — shares, cashflow, P&L per agent per market
- Resolution engine — accept winning outcome, halt trading
- Settlement engine — compute payouts, generate `SettlementReport` with deterministic hash
- Settlement invariant tests — bounded loss, payout conservation, P&L correctness
- End-to-end lifecycle test — create → commit → trade → resolve → settle → verify P&L
- New exceptions: `TradingHalted`, `InsufficientBalance`, `InsufficientShares`
- 25+ new Sprint 2 tests
...
## 8b. Acceptance Criteria (Sprint 2)

1. `TradingEngine.execute_trade()` correctly computes cost via `LMSREngine.trade_cost()` and updates `MarketState.x`
2. `TradingEngine.quote()` returns exact trade cost without executing
3. Trading rejected when market is not in TRADING phase (`TradingHalted`)
4. Sell rejected when agent has insufficient shares (`InsufficientShares`)
5. Buy rejected when agent has insufficient balance (`InsufficientBalance`)
6. Execution is atomic — no state mutation on validation failure
7. `PositionManager` correctly accumulates shares and net cashflow per agent per market
8. `PositionManager.compute_settlement_payout()` pays winning shares 1:1, losing shares 0
9. Resolution halts trading and records winning outcome
10. Settlement computes correct per-agent payouts
11. Market maker P&L never exceeds `-b * ln(n)` (bounded loss guarantee)
12. `SettlementReport` hash is deterministic (same inputs → same hash)
13. End-to-end lifecycle works: create → commit → trade → resolve → settle with correct P&L
14. All existing tests pass (Sprint 1 63 tests + pipeline)
15. All Cycle-009 MCP tests pass
16. 25+ new Sprint 2 tests pass
