# SDD: LMSR Market Engine (Local Mode) — Sprint 1
...
```
backend/market/
├── lmsr.py          ← Pure math (stateless)
├── state.py         ← Data structures (MarketState, MarketPhase, FeeSchedule)
├── lifecycle.py     ← State machine transitions (mutates MarketState)
├── commitment.py    ← Market parameter commitment hash
