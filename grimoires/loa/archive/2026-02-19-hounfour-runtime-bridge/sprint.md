...
### Task 8.1: Create invariants declaration schema [DONE]
...
**Acceptance Criteria**:
- [x] Schema defines `invariants` array with `id`, `description`, `properties` (string array of formal property expressions), `verified_in` (array of repo+file+function references)
- [x] Schema validates `severity` field: `"critical"` (must never be violated), `"important"` (should be verified), `"advisory"` (best-effort)
- [x] Schema validates `category` field: `"conservation"`, `"monotonicity"`, `"ordering"`, `"bounded"`, `"idempotent"`
- [x] JSON Schema draft 2020-12 compatible
- [x] Example invariant validates against schema

### Task 8.2: Declare Hounfour economic invariants [DONE]
...
**Acceptance Criteria**:
- [x] INV-001: Conservation — `sum(input_costs) == sum(distributed_costs) + sum(remainders)`, severity: critical
  - verified_in: `loa:pricing.py:RemainderAccumulator.add`, `loa:pricing.py:calculate_cost_micro`
- [x] INV-002: Non-negative spend — `daily_spend >= 0` at all times, severity: critical
  - verified_in: `loa:budget.py:pre_call_atomic`, `loa:ledger.py:update_daily_spend`
- [x] INV-003: Deduplication — `len(unique(interaction_ids)) == len(ledger_entries_for_interactions)`, severity: important
  - verified_in: `loa:budget.py:post_call`, `loa:ledger.py:append_ledger`
- [x] INV-004: Budget monotonicity — daily spend counter only increases within a day (never decremented), severity: critical
  - verified_in: `loa:budget.py:post_call`, `loa:ledger.py:update_daily_spend`
- [x] INV-005: Trust boundary — no model exceeds its trust_scopes at runtime, severity: critical
  - verified_in: `loa:resolver.py:resolve_execution`, `loa:model-permissions.yaml`
- [x] Cross-repo references annotated with `protocol: loa-hounfour@7.0.0` for ecosystem traceability
- [x] YAML validates against schema from Task 8.1

### Task 8.3: Implement invariant verification script [DONE]
...
**Acceptance Criteria**:
- [x] Reads `grimoires/loa/invariants.yaml`
- [x] For each invariant, for each `verified_in` entry where `repo == "loa"`:
  - Verify the file exists
  - Verify the function/class exists in the file (grep for definition)
  - Report PASS/FAIL per invariant
- [x] For cross-repo references (repo != "loa"): report SKIP with note (verified in external CI)
- [x] Exit code 0 if all local invariants pass, 1 if any fail
- [x] Output format compatible with `butterfreezone-validate.sh` pattern
- [ ] Integrates with `quality-gates.bats` as an optional check

### Task 8.4: Add invariant verification to existing test suite [DONE]
...
**Acceptance Criteria**:
- [x] Test script finds all declared invariants in valid codebase
- [x] Test script detects missing function (simulate by declaring non-existent function reference)
- [x] Test script detects missing file
- [x] Test script handles empty invariants.yaml gracefully
- [x] Test cross-repo references are SKIPped (not FAILed)
- [x] Test exit codes: 0 for all-pass, 1 for any-fail

### Task 8.5: Investigate eval regression 50% pass rate [PARTIAL — DEFERRED]
...
**Note**: Script is structurally complete but cannot execute — `evals/` directory (harness + regression tasks) exists in upstream Loa repo, not this mounted context. Deferred to upstream execution.

**Acceptance Criteria**:
- [x] Script runs each failing eval task 4 times, recording pass/fail per trial
- [x] Output: per-task breakdown showing which trial(s) pass and which fail
- [x] If pattern is "trial 1 always passes, trial 2 always fails" → report as HARNESS_BUG
- [x] If pattern is truly random 50/50 → report as FLAKY
- [x] If pattern is "always fails" → report as REGRESSION
- [x] Analysis saved to `.run/eval-regression-analysis.json`
- [ ] Document findings in NOTES.md *(blocked: needs execution results)*

### Task 8.6: Fix eval harness (conditional on Task 8.5 findings) [DEFERRED]

**Note**: Blocked by Task 8.5 execution. Deferred to upstream Loa repo where eval infrastructure exists.
