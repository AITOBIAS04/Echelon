...
// Transform API operation response (snake_case) to component format (camelCase)
const transformOperation = (apiOp) => ({
  id: apiOp.id,
  codename: apiOp.codename,
  description: apiOp.description,
  difficulty: apiOp.difficulty,
  timeRemaining: apiOp.time_remaining,
  expires_at: apiOp.expires_at,
  intelSources: apiOp.intel_sources?.map(s => ({
    id: s.id,
    name: s.name,
    accuracy: s.accuracy * 100, // Convert from 0-1 to 0-100
    cost: s.cost,
  })) || [],
  relatedMarkets: apiOp.related_markets || [],
  status: apiOp.status,
  participants: apiOp.participants,
  totalVolume: apiOp.total_volume,
});

// Enhanced fetcher for operations
const fetchOperations = async (url) => {
  const res = await fetch(url);
  const data = await res.json();
  return Array.isArray(data) ? data.map(transformOperation) : [];
};

// Transform intel packet response
const transformIntelPacket = (apiPacket) => ({
  id: apiPacket.id,
  topic: apiPacket.topic,
  encrypted_preview: apiPacket.preview,
  price_usdc: apiPacket.price_usdc,
  confidence: apiPacket.confidence,
  source_agent: apiPacket.source_agent,
});

const fetchIntel = async (url) => {
  const res = await fetch(url);
  const data = await res.json();
  return Array.isArray(data) ? data.map(transformIntelPacket) : [];
};
...
export default function SituationRoomPage() {
  ...
  // Fetch data from API - Updated to use new Operations API
  ...
  const handlePurchaseIntel = async (sourceId: string) => {
    try {
      await fetch(`${API_BASE}/api/intel/purchase`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ intel_id: sourceId, requester_wallet: "user" }),
      });
      mutateState();
    } catch (error) {
  ...
  const handleConfirmPosition = async (position: {
    ...
    try {
      await fetch(`${API_BASE}/api/operations/${selectedOperation?.id}/join`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          wallet_address: "user",
